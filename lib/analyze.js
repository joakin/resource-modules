...
function analyzeFiles (dir: string, jsFiles: string[], visitors: VisitorMap<State>, noisy: boolean): Promise<Analysis> {
  return getSources(dir, jsFiles)
    .then((files: Sources) => {
      let analysis: Analysis = { files: {} }
      Object.keys(files).forEach((f) => {
        analysis.files[f] = walkAst(visitors, files[f].source, f, noisy)
      })
      return analysis
    })
}
...
function walkAst (visitors: VisitorMap<State>, source: string, file: string, noisy?: boolean = false): FileAnalysis {
  try {
