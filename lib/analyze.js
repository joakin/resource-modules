...
export type Analysis = { [file: string]: FileAnalysis }

function analyzeFiles (dir: string, jsFiles: string[], visitors: VisitorMap<State>): Promise<{ files: Analysis }> {
  return getSources(dir, jsFiles)
    .then((files: Sources) => {
      let analysis: Analysis = {}
      Object.keys(files).forEach((f) => {
        analysis[f] = walkAst(visitors, files[f].source, f)
      })
