...
declare interface acorn$Literal {
  loc: acorn$Location, type: 'Literal', raw: string, value: string
}

declare interface acorn$Identifier {
  loc: acorn$Location, type: 'Identifier', name: string, value: string
}

declare interface acorn$MemberExpression {
  loc: acorn$Location, type: 'MemberExpression', object: acorn$Node, property: acorn$Node
}

declare interface acorn$CallExpression {
  loc: acorn$Location, type: 'CallExpression', callee: acorn$Node, arguments: acorn$Node[]
}

declare interface acorn$ArrayExpression {
  loc: acorn$Location, type: 'ArrayExpression', elements: acorn$Node[]
}

declare interface acorn$AssignmentExpression {
  loc: acorn$Location, type: 'AssignmentExpression', left: acorn$Node, right: acorn$Node
}

declare interface acorn$ObjectExpression {
  loc: acorn$Location, type: 'ObjectExpression', properties: acorn$Property[]
}
...
declare module "acorn/dist/walk" {
  ...
  export function ancestor<T>(ast: acorn$Node, visitors: VisitorMap<T>, base: null, initialState: T): void
}
