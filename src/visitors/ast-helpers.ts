...
export function isObjectAccess(
  obj: string,
  prop: string | null,
  node: Node
): boolean {
  ...
  if (object.type === "Identifier" && !named(obj, object)) {
    ...
    if (parts.length > 1 && !isObjectAccess(butLast.join("."), last, object))
      return false;
    else if (parts.length === 1 && !isObjectAccess(parts[0], null, object))
      return false;
  }
