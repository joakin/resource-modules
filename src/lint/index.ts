...
export default function lint (
  analysis: Analysis, coreAnalysis: Analysis, resourceModules: ResourceModules
): Errors {

  const fullAnalysis: Analysis = [coreAnalysis, analysis].reduce((all, ana) => ({
  ...
  Object.keys(analysis.files).forEach((file: string) => {
    const inModules: Module[] = getResourceModulesWithFile(file, resourceModules)
    ...
    if (inModules.length < 1) {
      ...
      errors.files[file] = {
        missingMessages: getMissingMessagesErrors(ana, inModules, resourceModules),
...
function getResourceModulesWithFile (file: string, resourceModules: ResourceModules): Module[] {
  return Object.keys(resourceModules)
    .filter((rk: string): boolean =>
      (resourceModules[rk].scripts || []).indexOf(file.replace(/^\//, '')) > -1)
